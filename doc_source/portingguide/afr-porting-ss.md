...
1. Start an echo server\.
   ...
   If you have not ported the TLS library to your platform, you can only test your Secure Sockets port using an unsecured echo server \(`<freertos>/tools/echo_server/echo_server.go`\)\. For instructions on setting up and running an unsecured echo server, see [Setting Up the Echo Server \(Without TLS\)](notls-echo-server.md)\.
