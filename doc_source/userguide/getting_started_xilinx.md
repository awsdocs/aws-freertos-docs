...
## Overview<a name="xilinx-overview"></a>
...
1. If you don't see the `aws_bsp`, `fsbl`, and `MicroZed_hw_platform_0` projects in the projects pane, repeat the previous steps starting from \#3 but with the root directory set to `freertos/vendors/xilinx`, and import `aws_bsp`, `fsbl`, and `MicroZed_hw_platform_0`\.
...
1. In **New String Substitution Variable**, for **Name**, enter **AFR\_ROOT**\. For **Value**, enter the root path of the `freertos/projects/xilinx/microzed/xsdk/aws_demos`\. Choose **OK**, and then choose **OK** to save the variable and close **Preferences**\.
